...
- `TrendbriefOpportunityRepository.upsertFromDetection` and `TrendbriefOpportunityLifecycleService`'s status+action writes are sequential, non-atomic writes, not wrapped in `runBatch` (`src/db/runBatch.ts`). Acceptable for TB-001's single-operator execution model; a concurrent-writer or scheduled-production version of this slice should wrap both in a transaction. Beyond atomicity, a large analysis run (up to ~1000 qualifying evidence rows, each producing several sequential DB round trips) can issue thousands of sequential statements in one Worker invocation — acceptable at TB-001's single-operator scale, but a future production-scheduled version of this slice should batch writes via `runBatch`/`executeInBatches` (both already used elsewhere in this codebase, e.g. `src/server/features/audit/repositories/AuditRepository.ts`) for both the atomicity and the round-trip-volume reasons.
